...
public class AdminReservationController {
    ...
    @Operation(summary = "전체 예약 조회",
            description = "현재 서비스에서 생성된 임시 예약(입금 안된 것)을 제외한 모든 예약을 검색합니다. <br> " +
    ...
    @Operation(summary = "임시 예약 전체 조회", description = "현재 서비스에서 생성된 임시 예약(입금 안된 것)만을 검색합니다")
    @PreAuthorize("hasRole('ADMIN')")
    @GetMapping("/temp-reservations")
    public CustomResponse<PageResponse<ReservationResDTO.TempReservationDetailResDTO>> getTempReservationList(
            @RequestParam(defaultValue = "0") int page,
            @RequestParam(defaultValue = "10") int size
    ) {
        Pageable pageable = PageRequest.of(page, size, Sort.by("createdAt").descending());
        return CustomResponse.onSuccess(PageResponse.of(adminReservationQueryService.getTempReservationList(pageable)));
    }
